...
from SOS_Aer_graphe import graphe_flux, graphe_diffusivity, graphe_heating_rate, graphe_successive_dif
...
def SOS_Aer():
    ...
    z_up = 15 # Upper boundary of aerosol layer (in km)
    z_down = 14 # Lower boundary of aerosol layer (in km)
    ...
    tauStar_aer = 0.0075 # Aerosols optical depth
    ...
    alb_mix = (alb_atm * dtau_atm + alb_aer * dtau_aer) / (dtau_aer + dtau_atm)
    ...
    aer_phase_fun = 'wildfire'  # 'iso', 'hg', 'fwc', 'mie', 'rayleigh', 'wildfire' 'eva'
    ...
    for t in range(idx_up):        
        ...
        with warnings.catch_warnings():
            ...
            I1[t, m_arr] = (mu0/(mu0+mu[m_arr])) * (alb_atm/(4*np.pi)) * P0_atm[m_arr] * F0 * (np.exp(-tau[t] / mu0) - np.exp(+tau[t]/mu[m_arr]))
        # case µ = 0
        I1[t, nb_angles-1] = (alb_atm/(4*np.pi)) * (mu0/(mu0+mu[nb_angles-1])) * P0_atm[nb_angles-1] * F0 * np.exp(-tau[t] / mu0)
        # Case |µ|=µ0        
        if np.any(mask_mu0):
            I1[t, m_arr[mask_mu0]] = (alb_atm/(4*np.pi)) * P0_atm[m_arr[mask_mu0]] * F0 * np.exp(-tau[t] / mu0) * tau[t] / mu0

    for t in range(idx_up, idx_down+1):
        # general case: µ != 0 and µ0
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", category=RuntimeWarning)
            sum_exp = (np.exp(-tau[t] / mu0) - np.exp(-tau[idx_up-1] / mu0) * np.exp(+(tau[t]-tau[idx_up-1])/mu[m_arr]))
            I1[t, m_arr] = I1[idx_up-1, m_arr] * np.exp((tau[t]-tau[idx_up-1])/mu[m_arr]) + (mu0/(mu0+mu[m_arr])) * (alb_mix/(4*np.pi)) * P0_mix[m_arr] * F0 * sum_exp[m_arr]
        # case µ = 0
        I1[t, nb_angles-1] = (alb_atm/(4*np.pi)) * (mu0/(mu0+mu[nb_angles-1])) * P0_atm[nb_angles-1] * F0 * np.exp(-tau[t] / mu0)
        # Case |µ|=µ0
        if np.any(mask_mu0):
            I1[t, m_arr[mask_mu0]] = I1[idx_up-1, m_arr[mask_mu0]] * np.exp((tau[t]-tau[idx_up-1])/mu[m_arr[mask_mu0]]) + (alb_atm/(4*np.pi)) * P0_atm[m_arr[mask_mu0]] * F0 * np.exp(-tau[t] / mu0) * (tau[t]-tau[idx_up-1]) / mu0

    for t in range(idx_down+1, nb_layers):
        # general case: µ != 0 and µ0
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", category=RuntimeWarning)
            sum_exp = (np.exp(-tau[t] / mu0) - np.exp(-tau[idx_down] / mu0) * np.exp(+(tau[t]-tau[idx_down])/mu[m_arr]))
            I1[t, m_arr] = I1[idx_down, m_arr] * np.exp((tau[t]-tau[idx_down])/mu[m_arr]) + (mu0/(mu0+mu[m_arr])) * (alb_atm/(4*np.pi)) * P0_atm[m_arr] * F0 * sum_exp[m_arr]
        # case µ = 0
        I1[t, nb_angles-1] = (alb_atm/(4*np.pi)) * (mu0/(mu0+mu[nb_angles-1])) * P0_atm[nb_angles-1] * F0 * np.exp(-tau[t] / mu0)
        # Case |µ|=µ0
        if np.any(mask_mu0):
            I1[t, m_arr[mask_mu0]] = I1[idx_down, m_arr[mask_mu0]] * np.exp((tau[t]-tau[idx_down])/mu[m_arr[mask_mu0]]) + (alb_atm/(4*np.pi)) * P0_atm[m_arr[mask_mu0]] * F0 * np.exp(-tau[t] / mu0) * (tau[t]-tau[idx_down]) / mu0
    ...
    for t in range(idx_down+1, nb_layers):
        # case µ = 0
        I1[t, nb_angles] = (alb_atm/(4*np.pi)) * (mu0/(mu0+mu[nb_angles])) * P0_atm[nb_angles] * F0 * np.exp(-tau[t] / mu0)
        # general case: µ != 0
        sum_exp = (np.exp(-tau[t] / mu0) - np.exp(-tau[nb_layers-1] / mu0) * np.exp(-(tau[nb_layers-1]-tau[t])/mu[m_arr]))
        I1[t, m_arr] = grd_alb * I1[nb_layers-1, nb_angles-1-(m_arr-nb_angles)] * np.exp(-(tau[nb_layers-1]-tau[t])/mu[m_arr]) + (mu0/(mu0+mu[m_arr])) * (alb_atm/(4*np.pi)) * P0_atm[m_arr] * F0 * sum_exp
    
    for t in range(idx_up, idx_down+1):
        # case µ = 0
        I1[t, nb_angles] = (alb_mix/(4*np.pi)) * (mu0/(mu0+mu[nb_angles])) * P0_mix[nb_angles] * F0 * np.exp(-tau[t] / mu0)
        # general case: µ != 0
        sum_exp = (np.exp(-tau[t] / mu0) - np.exp(-tau[idx_down+1] / mu0) * np.exp(-(tau[idx_down+1]-tau[t])/mu[m_arr]))
        I1[t, m_arr] = I1[idx_down+1, m_arr] * np.exp(-(tau[idx_down+1]-tau[t])/mu[m_arr]) + (mu0/(mu0+mu[m_arr])) * (alb_mix/(4*np.pi)) * P0_mix[m_arr] * F0 * sum_exp
    
    for t in range(idx_up):
        # case µ = 0
        I1[t, nb_angles] = (alb_atm/(4*np.pi)) * (mu0/(mu0+mu[nb_angles])) * P0_atm[nb_angles] * F0 * np.exp(-tau[t] / mu0)
        # general case: µ != 0
        sum_exp = (np.exp(-tau[t] / mu0) - np.exp(-tau[idx_up] / mu0) * np.exp(-(tau[idx_up]-tau[t])/mu[m_arr]))
        I1[t, m_arr] = I1[idx_up, m_arr] * np.exp(-(tau[idx_up]-tau[t])/mu[m_arr]) + (mu0/(mu0+mu[m_arr])) * (alb_atm/(4*np.pi)) * P0_atm[m_arr] * F0 * sum_exp
    ...
    while max(max(In[0,nb_angles:]/I[0,nb_angles:]), max(In[nb_layers-1,:nb_angles]/I[nb_layers-1,:nb_angles])) >= 0.0001:
        ...
        for t in range(nb_layers):
            if t>=idx_up and t<=idx_down:
                Jn[t,:] = (alb_mix/4) * np.trapz(P_mix[:,::-1]*In_1[t,:], mu, axis=1)
            else: 
