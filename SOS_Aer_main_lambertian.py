...
def SOS_Aer():
    ...
    for t in range(idx_up):
        ...
        with warnings.catch_warnings():
            ...
            # Scattered after reflecting on the surface
            scatt_surface = np.zeros(nb_angles)
            for m in m_arr:
                scatt_surface[m] = np.trapz((mu[nb_angles:]/(mu[nb_angles:]-mu[m])) * (alb_atm/(4*np.pi)) * P_atm[m, 2*nb_angles-1-m_arr] * (2*F0*grd_alb*np.exp(-tauStar_tot/mu0)) * (np.exp(-(tauStar_tot-tau[t])/mu[nb_angles+1:]) - np.exp(-(tauStar_tot-tau[idx_up-1])/mu[nb_angles+1:])*np.exp(-(tau[idx_up-1]-tau[t])/mu[m])), mu[nb_angles+1:])  
            I1[t, m_arr] = scatt_before + scatt_direct + scatt_surface
        ## case µ = 0
        # Alreday scattered
        scatt_before = 0
    ...
    while max(max(In[0,nb_angles:]/I[0,nb_angles:]), max(In[nb_layers-1,:nb_angles]/I[nb_layers-1,:nb_angles])) >= 0.0001:
        ...
        for t in range(idx_down+1, nb_layers):
            ...
            if tau[nb_layers-1]/mu[nb_angles+1] >= 50:
                In[t, m_arr] = - 2 * grd_alb * np.trapz(In[nb_layers-1, nb_angles-1-(m_arr-nb_angles)]*mu[nb_angles-1-(m_arr-nb_angles)], mu[nb_angles-1-(m_arr-nb_angles)]) * np.exp(-(tau[nb_layers-1]-tau[t])/mu[m_arr]) + np.trapz(Jn[t:, m_arr] * (exp_term / mu[m_arr]), tau[t:], axis=0)
            else:
                In[t, m_arr] = - 2 * grd_alb * np.trapz(In[nb_layers-1, nb_angles-1-(m_arr-nb_angles)]*mu[nb_angles-1-(m_arr-nb_angles)], mu[nb_angles-1-(m_arr-nb_angles)]) * np.exp(-(tau[nb_layers-1]-tau[t])/mu[m_arr]) + np.trapz(Jn[t:, m_arr] * exp_term, tau[t:], axis=0) / mu[m_arr]
